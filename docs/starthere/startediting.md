...
`docs/site_documentation.pdf` isn't built by `zensical serve` or `zensical build` - it has its own build script, `build_pdf.py`, a zendoc-specific extension on top of Zensical (see [Customise PDF format](customise.md#customise-pdf-format) for what it is and how to customise its output; this section just covers running it).
