...
CSS in extra.css vs PDF CSS generated by prodockit.pdf.build.build_pdf() -
since Pandoc's HTML has no .md-typeset wrapper for the website's own rules
to match against -
...
def test_pdf_uses_the_documented_default_fonts_when_unset(zensical_config):
    ...
    rather than reimplemented here (prodockit pdf doesn't hardcode these -
...
def _css_rule_value(css_text, selector, property_name, occurrence=0):
    ...
    in css_text - prodockit.pdf.css.build_css() has two branches (if
    reference_style_global / else) that both define "p.reference +
