...
| `zensical_basics` | The Zensical-specific extensions documented in [Zensical basics](zensicalbasics.md) - admonitions (all 12 configured types, custom/empty titles, nesting, collapsible details), code blocks (line highlighting, line numbers, titles, annotations, inline highlighting), content tabs (nested, anchor ids), images (figure/figcaption, alignment, lazy loading, light/dark hash fragments), diagrams (all 5 officially-supported Mermaid types), footnotes, formatting (mark, keys), icons/emojis, maths (arithmatex wrapping), task lists, and tooltips/abbreviations - cross-checked against the upstream `zensical.org/docs/authoring/*` pages this page itself links to, not just its own worked examples. |
